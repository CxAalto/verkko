...
def savefig(fig, name, extensions, verbose=False):
    """Save figure.
    ...
    for ext in extensions:
        ...
        if ext == 'eps':
            # Save as pdf, convert to eps and remove the original pdf.
            fig.savefig(name + ".pdf")
            os.popen("pdftops -eps %s" % (name + ".pdf",))
            if 'pdf' not in extensions:
                os.popen("rm %s" % (name + ".pdf",))
        else:
...
def get_rcParams(fig_width_cm, fig_ratio = 0.8, font_sizes = None):
    ...
    font_sizes: dictionary
        The sizes of the fonts used in the figure. Default is size 8
        for everything else expect 10 for the title. Possible keys are
        'label', 'title', 'text', 'legend', 'tick', the meaning should
        be self explanatory.
    """
    ...
    for k in default_font_sizes:
        if k not in font_sizes:
            font_sizes[k] = default_font_sizes[k]
