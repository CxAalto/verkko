...
def plot_alluvial(ax, ribbon_size_matrix, ribbon_label_matrix=None,
    ...
    """Plot 2-step alluvial diagram to the ax given as parameter.

    Arguments:
    ax -- a matplotlib.axes object
    ribbon_size_matrix -- a numpy array describing the
    ribbon_label_matrix -- label text for the ribbons
    module_colors_1 -- colors for the first (left) modules
                        (defaulting to gray)
    module_colors_1 -- colors for the second (right) modules
                        (defaulting to gray)
    ribbon_bglim   --  if ribbon contains less than this number of nodes it
                        is drawn on the background to avoid visual clutter
    stable_ribbon_sizes_1 -- a matrix describing the number of shaded nodes
                            of each module for the first moduless
                            (defaulting to no shading)
    stable_ribbon_sizes_2 -- a matrix describing the number of shaded nodes
                            of each module for the second modules
                            (defaulting to no shading)
    ribbon_label_size -- the font size for the ribbon labels
    ribbon_label_HA -- the horizontal alignment of the ribbon labels
    ...
    for i, iteration_data in enumerate(iteration_list):
        ...
        for j in range(len(module_sizes)):
            ...
            if stable_ribbon_sizes_1 is not None:
                rect = Rectangle((rect_x_start+i*mw*(1-mwnsf), current_y),
    ...
    for i in range(len(ribbon_size_matrix)):
        for j in range(len(ribbon_size_matrix[i])):
            ...
            if stable_ribbon_sizes_1 is not None and \
                ...
                ax.add_patch(rect)
                rect = Rectangle((1-horizontal_pad_lr-module_width, y2_low),
            ...
            if ribbon_label_matrix is not None:
                ...
                if s != "":
                    ax.text(horizontal_pad_lr+module_width, (ystart1+yend1)/2.,
...
def _plot_ribbon_using_bezier(ax, zorder, points1, points2, color1="gray",
    ...
    for points in [points1, points2]:
        ...
        allPoints = (p1[:, np.newaxis]*(1-tRange)**3+p2[:, np.newaxis] *
                    ...
                    (3*(1-tRange)*tRange**2)+p4[:, np.newaxis] * tRange**3)
        xpoints = allPoints[0]
